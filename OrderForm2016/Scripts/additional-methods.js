...
(function(factory) {
    ...
    */
    ...
    $.validator.addMethod("notEqualTo", function (value, element, param) {
        var target = $(param);
        var opts = $.data(element.form, 'validator').settings;
        if (opts.onfocusout) {
            target.unbind(".validate-notEqualTo").bind("blur.validate-notEqualTo", function() {
                $(element).valid();
            });
        }
        if (value === "US" && target.val() === "US") {return true;}
        return value !== target.val();
    }, $.validator.format("The two values must be different"));
    ...
    $.validator.addMethod("dateLessThan", function (value, element, params) {
    	...
    	return isNaN(value) && isNaN(endDate) || (Number(value) < Number(endDate));
    }, $.validator.format("This date must be earlier than the trips return date"));
    ...
    $.validator.addMethod("dateGreaterThan", function (value, element, params) {
    	...
    	if (!/Invalid|NaN/.test(new Date(value))) {
    		...
    		return new Date(value) > (startDate ? new Date(startDate) : Date.now());
			}
    	return isNaN(value) && isNaN(startDate) || (Number(value) > Number(startDate));
    }, $.validator.format("This date must be later than the trips departure date"));
		...
		/** (https://christierney.com/2011/06/30/jquery-validate-better-date-method/)
